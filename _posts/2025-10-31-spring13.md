...
layout: single
title: "Login 기능구현"
excerpt: "SecurityConfig,CSRF,Lambda표현식,BCryptPasswordEncoder을 이용한 비번hasing,원래있던 "
...
만약 없으면 에러메세지로 "그런 아이디 없음" 있으면 user정보(user.getUsername(),user.getPassword, authorities)를 return ➤ 유저아디,비번 일치하면 유저가 제출한거랑 조회하고 맞으면 통과.
...
class CustomUser extends User {
  ...
  **auth.getPrincipal().displayName** 이런거 쓰면 되는데 안됄텐데 
  ...
  displayname인 park이 출력되는걸 볼 수 있다.
